#!/usr/bin/env python3
"""
Query alternative models to replace failed ones
"""
...
# Alternative models to replace failed Perplexity and OpenAI models
ALTERNATIVE_MODELS = {
    # Replace perplexity/sonar-deep-research - use Alibaba's deep research model
    "alibaba/tongyi-deepresearch-30b-a3b": "Research Tier - Alibaba Deep Research (replacement)",
    # Replace perplexity/sonar-pro - use Mistral Large
    "mistralai/mistral-large-2512": "Research Tier - Mistral Large (replacement)",
    # Replace openai/gpt-5.2 - use Meta Llama 4
    "meta-llama/llama-4-maverick": "Reasoning Tier - Meta Llama 4 Maverick (replacement)",
}
...
async def run_alternative_queries():
    """Run alternative model queries in parallel"""
    
    print(f"Querying {len(ALTERNATIVE_MODELS)} alternative models...")
    print("-" * 60)
    
    async with aiohttp.ClientSession() as session:
        tasks = [
            query_model(session, model_id, tier)
            for model_id, tier in ALTERNATIVE_MODELS.items()
        ]
        
        results = await asyncio.gather(*tasks)
    
    # Load existing results - only keep successful ones
    with open("/home/ubuntu/model_results.json", "r") as f:
        all_results = json.load(f)
    
    # Filter to only successful results
    successful_results = [r for r in all_results["results"] if r["status"] == "success"]
    ...
    # Update results
    all_results["results"] = successful_results
    all_results["successful"] = len(successful_results)
    all_results["failed"] = 0  # Reset since we're only keeping successes
    all_results["total_models"] = len(successful_results)
    ...
    print("-" * 60)
    print(f"Total successful results: {len(successful_results)}")
...
if __name__ == "__main__":
    asyncio.run(run_alternative_queries())
